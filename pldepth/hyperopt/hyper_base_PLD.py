...
def perform_base_PLD(pars=None):
    with wandb.init(config=pars, settings=wandb.Settings(_disable_stats=True)):
        ...
        val_imgs_ds, val_gts_ds, val_cons_masks, = dao.get_validation_dataset(size=ds_size)
        ...
        #wandb.run.summary["base_test_error"] = err
        wandb.log({'base_val_loss': err})
        ...
        ferr = calc_err(model, test_img, test_gt)
        wandb.run.summary["final_val_loss"] = ferr
        wandb.log({'val_loss': ferr})
