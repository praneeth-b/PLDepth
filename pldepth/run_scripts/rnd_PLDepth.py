...
def perform_pldepth_experiment(model_name, epochs, batch_size, seed, ranking_size, rankings_per_image, initial_lr,
    ...
    # Compile model
    lr_sched_prov = LearningRateScheduleProvider(init_lr=initial_lr, steps=[5, 8,13,25], warmup=warmup, multiplier=lr_multi)
    ...
    model_params.set_parameter("rankings_per_image", 2)   ## to match with active sampling

    print("****** new sampling strategy******: ", model_params.get_parameter("sampling_strategy"))
    wandb.log({'new-rpi': model_params.get_parameter("rankings_per_image")})
    data_path_n = config["DATA"]["HR_WSI_10K_PATH"]
    dao_r = HRWSITFDataAccessObject(data_path_n, model_input_shape, seed=seed)
    test_imgs_ds, test_gts_ds, test_cons_masks = dao_r.get_training_dataset(size=5000)
    ...
    n_epochs = epochs+6
    lr_sched_prov.init_lr = 0.007*11
